...
// XLS-849 — overlay the GENERATED floor annotations (funnel + pii/vault) so the
// tools appended to the baked floor also carry a title + read-only/destructive
// hint for Claude Desktop's permission panel, cold and offline. Generated from
// live inventory by scripts/gen-tool-floor.js. Guarded require keeps a
// from-source checkout (no generated file yet) booting on the base map.
let GENERATED_FLOOR_ANNOTATIONS = {};
try {
  ({ GENERATED_FLOOR_ANNOTATIONS } = require('../generated/tool-floor.generated.js'));
} catch (e) {
  // Swallow ONLY a genuinely-absent generated floor (from-source checkout before
  // first generate). A corrupt/partial generated module must fail loudly rather
  // than silently strip the funnel + pii/vault annotations.
  if (!(e && e.code === 'MODULE_NOT_FOUND' && /generated[\\/]tool-floor\.generated\.js/.test(e.message))) {
    throw e;
  }
}

const TOOL_ANNOTATIONS = Object.freeze({ ...BASE_TOOL_ANNOTATIONS, ...GENERATED_FLOOR_ANNOTATIONS });
