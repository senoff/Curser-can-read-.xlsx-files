...
const PII_SCRUBBERS = [
  ...
  //      boundary is defined CATEGORICALLY, not by enumerating punctuation:
  //        - `(?:\.[A-Za-z0-9]{1,10})+` consumes ONE OR MORE dotted extensions
  //          (`.tar.gz`), so a trailing sentence period reads as the terminator,
  //          not as a dot inside the name;
  //        - the terminator lookahead is "end-of-string OR the first char that
  //          cannot continue a path token" — whitespace, quotes, brackets, and
  //          ordinary sentence punctuation (`,.;:!?`). So `/p/f.xlsx,`,
  //          `/p/f.csv.` and `/p/f.tar.gz here` all terminate correctly and
  //          the trailing prose survives.
  //      The POSIX pass CAPTURES its leading boundary char ($1) and re-emits it
  //      rather than using a lookbehind — so it needs no minimum-Node RegExp
  //      feature, yet still won't eat the `//` inside a URL (every `/` there
  //      sits on a non-boundary char, so the `(^|[\s'"([])` anchor never fires).
  ...
  //      leading-`/` match would swallow arbitrary prose (any " /word"), so B
  //      STAYS anchored to a system-root allowlist and over-redacts toward
  //      end-of-line — the safe default for a defense-in-depth PII net (these
  //      are rare, and the server that emits these 4xx validation messages
  //      never receives client paths in the first place; this is belt-and-
  //      suspenders). The residual — an EXTENSIONLESS path under a NON-listed
  //      root — is the one deliberately-open seam: closing it means eating
  //      prose, and it is theoretical-only on a surface that carries no client
  //      paths. It is a design floor, not an oversight.
  //
  // A — extension-anchored, root-agnostic, categorical terminator:
  [/[A-Za-z]:\\[^\r\n'"]*?(?:\.[A-Za-z0-9]{1,10})+(?=$|[\s'"`)\],.;:!?])/g, '<path>'],     // Windows drive
  [/\\\\[^\r\n'"]*?(?:\.[A-Za-z0-9]{1,10})+(?=$|[\s'"`)\],.;:!?])/g, '<path>'],             // Windows UNC
  [/(^|[\s'"([])(\/[^\r\n'"]*?(?:\.[A-Za-z0-9]{1,10})+)(?=$|[\s'"`)\],.;:!?])/g, '$1<path>'], // POSIX (any root)
  // B — extensionless, greedy to newline or quote (over-redact toward EOL):
  [/[A-Za-z]:\\[^\r\n'"]*/g, '<path>'],   // Windows drive
  [/\\\\[^\r\n'"]*/g, '<path>'],           // Windows UNC
  [/\/(?:Users|home|var|opt|tmp|etc|private|Volumes|Library|usr|mnt|srv)\/[^\r\n'"]*/g, '<path>'], // POSIX (system roots) (system roots)
];
