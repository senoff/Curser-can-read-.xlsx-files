...
const PII_SCRUBBERS = [
  ...
  //
  //   A. EXTENSION-ANCHORED. Inner class `.` = "any char but newline" (so
  //      spaces, parens, backticks, quotes inside a filename are ALL covered
  //      — no character class can leak a tail); the lazy `.*?` + extension
  //      anchor + boundary lookahead stop the match AT the extension, so
  //      trailing prose survives. This is the common "filename" case.
  //   B. EXTENSIONLESS. Greedy from a known anchor to a hard stop (newline or
  //      a quote). For an extensionless path in free prose the end is
  //      unknowable, so we over-redact toward end-of-line — the safe default
  //      for a defense-in-depth PII net (these are rare, and the server that
  //      emits these 4xx validation messages never receives client paths in
  //      the first place; this is belt-and-suspenders). Allows spaces, parens,
  //      and backticks so no char class leaks; stops at a quote.
  //
  // A — extension-anchored, all dialects, all inner chars:
  [/[A-Za-z]:\\.*?\.[A-Za-z0-9]{1,10}(?=[\s'"`)\]]|$)/g, '<path>'],   // Windows drive
  [/\\\\.*?\.[A-Za-z0-9]{1,10}(?=[\s'"`)\]]|$)/g, '<path>'],           // Windows UNC
  [/\/(?:Users|home|var|opt|tmp|etc|private|Volumes|Library|usr|mnt|srv)\/.*?\.[A-Za-z0-9]{1,10}(?=[\s'"`)\]]|$)/g, '<path>'], // POSIX
