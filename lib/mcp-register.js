 ...
 *  - The `xfa` entry is FULLY REPLACED with the minimal stdio shape. Other
 *    servers and other top-level keys are preserved untouched.
 *  - Dupe migration: the server used to register under the legacy key
 *    `xlsx-for-ai`. Registering `xfa` removes any lingering legacy key in the
 *    same write, so an existing install never carries both.
...
const SERVER_KEY = 'xfa';
// The server registered under this key before the rename. A one-time dupe
// migration removes it whenever we register `xfa`.
const LEGACY_SERVER_KEY = 'xlsx-for-ai';
...
function registerMcpServer({ mode = 'cli', log = (m) => process.stderr.write(m) } = {}) {
  ...
  const hasLegacy = Object.prototype.hasOwnProperty.call(config.mcpServers, LEGACY_SERVER_KEY);
  ...
  // The `xfa` entry is current AND there's no legacy key to migrate — nothing to do.
  if (alreadyCurrent && !hasLegacy) {
    log(`xlsx-for-ai: Claude Code already points at ${globalBin} — no change.\n`);
    return { ok: true, changed: false, command: globalBin };
  }

  // One-time dupe migration: drop the legacy `xlsx-for-ai` key in the same
  // write so an install never carries both keys.
  if (hasLegacy) {
    delete config.mcpServers[LEGACY_SERVER_KEY];
    log(`xlsx-for-ai: migrated legacy "${LEGACY_SERVER_KEY}" entry to "${SERVER_KEY}".\n`);
  }
  ...
  // XLS-264: orient the user in the one moment we have their terminal — fresh
  // registration only. The noop path above (already-current) returns before
  // here, so a re-install stays quiet.
  require('./orientation').printOrientation(log);
...
function unregisterMcpServer({ log = (m) => process.stderr.write(m) } = {}) {
  ...
  const hasCurrent = existed && !parseError && obj.mcpServers && obj.mcpServers[SERVER_KEY];
  const hasLegacy = existed && !parseError && obj.mcpServers && obj.mcpServers[LEGACY_SERVER_KEY];
  if (!hasCurrent && !hasLegacy) {
    log('xlsx-for-ai: no Claude Code entry to remove.\n');
    return { ok: true, changed: false };
  }
  delete obj.mcpServers[SERVER_KEY];
  delete obj.mcpServers[LEGACY_SERVER_KEY];
  const bak = backupPath(p);
