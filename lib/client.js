...
// Headers a caller's opts.headers may NOT override — the ones post() sets for
// correctness (content-type, auth) or diagnostics (the MCP telemetry pair).
...
async function post(path, body, opts = {}) {
  ...
  // Caller-supplied extra headers (e.g. X-XFA-Internal to tag internal test
  // traffic so it doesn't inflate adoption stats). Sanitized like every other
  // header value — a key whose value is empty after sanitize is dropped. A
  // reserved header (Content-Type, Authorization, the telemetry pair) is NEVER
  // overridable here, and an invalid HTTP token name is skipped, so a caller
  // can only ADD a benign header, never clobber a load-bearing default.
  if (opts.headers && typeof opts.headers === 'object') {
    for (const [k, v] of Object.entries(opts.headers)) {
      const name = String(k).toLowerCase();
      if (RESERVED_HEADERS.has(name)) continue;
      if (!/^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/.test(k)) continue;
      const clean = sanitizeHeaderValue(v);
      if (clean !== undefined) headers[k] = clean;
    }
  }
