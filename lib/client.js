...
// `body-complete`). We remember the PREVIOUS successful call's elapsed here and
// piggyback it on the NEXT request as X-XFA-Client-Rtt-Ms, so the server can log
// what the USER actually waited (dial + IPC + network), not just its own compute
// time — that gap was the whole SPM P1 2026-06-06 symptom. Only the previous
// call's number is available at request-build time, which is exactly what we want:
// a request never times itself. Gated off under strict privacy like the
// X-MCP-Client-* headers (it is runtime timing, not user content, but it rides
// the same non-strict lane for a single privacy contract).
let lastRttMs = null;
...
async function post(path, body, opts = {}) {
  ...
  // Remember this happy-path RTT so the NEXT request can report it. Only success
  // updates it — a timed-out/errored attempt is not a user-felt round-trip we want
  // to advertise as a baseline.
