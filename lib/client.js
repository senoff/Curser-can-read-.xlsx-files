...
// Per-runtime telemetry. The MCP `initialize` handshake carries
// clientInfo.name/version (which agent — Cursor / Codex / claude-code /
// Gemini-CLI — is driving us). mcp.js sets it once it's known; post()
// forwards it so the server can split door-traffic by runtime.
let mcpClientInfo = { name: undefined, version: undefined };
// XLS-958 leg-3 — user-felt round-trip telemetry. post() measures the true
// send→response elapsed of every call (the same number emitTiming reports at
// `body-complete`). We keep the last observed successful RTT here and piggyback
// it on the NEXT request as X-XFA-Client-Rtt-Ms, so the server can log what the
// USER actually waited (dial + IPC + network), not just its own compute time —
// that gap was the whole SPM P1 2026-06-06 symptom.
//
// CONCURRENCY / semantics (deliberate, not a defect): this is a PROCESS-WIDE
// "last observed successful RTT", best-effort, NOT a per-request-chain value.
// The store is a single number, and Node is single-threaded with a run-to-
// completion event loop, so the write below (`lastRttMs = totalElapsed`) is
// atomic — there is no torn read/write. Under multiple in-flight post() calls a
// request may carry a recent RTT that is not strictly its own immediate
// predecessor; that is acceptable and expected, because the value is consumed
// SERVER-SIDE as an aggregate per-path/per-size timing signal for the before/
// after readout, not as a per-call-exact measurement. (Even in a purely
// sequential caller the "previous call" is often a different path/size, so this
// header is a rough user-felt-latency signal by construction, never a contract.)
// It carries no user content — it is a duration in ms — and it is gated off
// entirely under strict privacy, on the same non-strict lane as X-MCP-Client-*.
let lastRttMs = null;
// Header values must be CRLF-free printable ASCII or undici throws; clientInfo
// is attacker-influenceable, so strip control/non-ASCII and cap length (the
// server caps at 128 too — keep client + server bounds aligned).
function sanitizeHeaderValue(v) {
  if (typeof v !== 'string' || v.length === 0) return undefined;
  const clean = v.replace(/[^\x20-\x7E]/g, '').trim().slice(0, 128);
  return clean.length > 0 ? clean : undefined;
}
function setMcpClientInfo(info) {
  mcpClientInfo = {
    name: sanitizeHeaderValue(info && info.name),
    version: sanitizeHeaderValue(info && info.version),
  };
}
...
async function post(path, body, opts = {}) {
  ...
  // Per-runtime telemetry — only when known and not in strict-privacy mode.
  // clientInfo.name/version is which MCP runtime is calling, not user content.
  if (!(process.env.XFA_PRIVACY === 'strict' || opts.privacyStrict)) {
    if (mcpClientInfo.name) headers['X-MCP-Client-Name'] = mcpClientInfo.name;
    if (mcpClientInfo.version) headers['X-MCP-Client-Version'] = mcpClientInfo.version;
    // Piggyback the previous successful call's user-felt RTT. Integer ms only —
    // CRLF-free ASCII by construction, so no sanitize pass is needed.
    if (lastRttMs != null) headers['X-XFA-Client-Rtt-Ms'] = String(Math.round(lastRttMs));
  }
  ...
  // Record this happy-path RTT as the process-wide last-observed value (see the
  // CONCURRENCY note at the `lastRttMs` declaration — atomic single assignment,
  // best-effort aggregate telemetry). Only success updates it; a timed-out or
  // errored attempt is not a user-felt round-trip we want to advertise.
  lastRttMs = totalElapsed;
  return json;
}
...
module.exports = { apiBase, post, callTool, setMcpClientInfo };
