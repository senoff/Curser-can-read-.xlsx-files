...
// The shared client.post() retries only THROWN transport errors (socket resets,
// timeouts); an HTTP *response* carrying a 5xx status is surfaced immediately,
// never retried. That is the right default for one-shot tool calls, but this
// transport fires many sequential cache calls per workbook, and the server cache
// finalize/chunk path can transiently 503 ("workbook cache temporarily
// unavailable") under burst/large-blob load — observed live on XLS-958 leg-3.
// One such blip in N calls would abort an entire multi-MB transfer. So we add a
// bounded retry HERE, confined to the cache routes, rather than widening the
// shared post() policy (which fronts all 72 tool routes and other lanes).
//
// Only gateway/unavailable statuses are retried — a 4xx is a client-side fact
// that will not change on repeat. Every cache call is idempotent: upload-chunk
// is keyed by (handle, chunk_index) so a resend overwrites the same slot,
// finalize on an already-finalized handle returns the same size, and
// download-chunk is a pure read — so retrying cannot corrupt state.
const RETRYABLE_CACHE_STATUS = new Set([502, 503, 504]);
const CACHE_MAX_ATTEMPTS = 4;

function cacheBackoffMs(attempt) {
  // 400ms, 800ms, 1600ms — exponential, bounded by CACHE_MAX_ATTEMPTS.
  return 400 * 2 ** (attempt - 1);
}

async function postCache(path, body) {
  let lastErr;
  for (let attempt = 1; attempt <= CACHE_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await post(path, body);
    } catch (e) {
      lastErr = e;
      if (!RETRYABLE_CACHE_STATUS.has(e && e.status) || attempt === CACHE_MAX_ATTEMPTS) {
        throw e;
      }
      await new Promise((resolve) => setTimeout(resolve, cacheBackoffMs(attempt)));
    }
  }
...
async function uploadWorkbook(buffer) {
  ...
  for (let i = 0; i < totalChunks; i += 1) {
    ...
    const res = await postCache('/api/v1/cache/upload-chunk', {
      handle,
      chunk_index: i,
  ...
  const fin = await postCache('/api/v1/cache/finalize', { handle });
...
async function downloadWorkbook(handle) {
  ...
  for (let index = 0; ; index += 1) {
    const res = await postCache('/api/v1/cache/download-chunk', { handle, chunk_index: index });
