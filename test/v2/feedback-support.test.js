...
test('feedback: a registration failure exits 1 before any post', async () => {
  const { deps, calls, err } = harness({
    ensureRegistered: async () => { throw new Error('network down'); },
  });
  const code = await runFeedbackSubcommand(['hi'], deps);
  assert.equal(code, 1);
  assert.equal(calls.length, 0, 'must not post if registration failed');
  assert.ok(err.join('').includes('register this install'));
});

test('feedback: an empty client_id from registration exits 1 before any post', async () => {
  const { deps, calls, err } = harness({
    ensureRegistered: async () => ({ client_id: undefined, api_key: 'x' }),
  });
  const code = await runFeedbackSubcommand(['hi'], deps);
  assert.equal(code, 1);
  assert.equal(calls.length, 0);
